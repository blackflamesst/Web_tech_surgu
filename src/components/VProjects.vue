<script setup>
const projects = [
  {
    img: {
      src: '/edukesha.png',
      alt: 'EduКеша'
    },
    link: {
      href: '#',
      title: 'EduКеша'
    },
    description: 'Разработали сервис для проведения онлайн-занятий'
  },
  {
    img: {
      src: '/donatelike.png',
      alt: 'DonateLike'
    },
    link: {
      href: '#',
      title: 'DonateLike'
    },
    description: 'Разработали телеграмм-бота для отправки платных реакций'
  },
  {
    img: {
      src: '/surgu.png',
      alt: 'go.surgu.ru'
    },
    link: {
      href: '#',
      title: 'go.surgu.ru'
    },
    description: 'Разработали сайт приемной комиссии СурГУ'
  }
]
</script>

<template>
  <section class="projects">
    <div class="container">
      <h2>
        Проекты
      </h2>
      <div class="row">
        <div class="col-sm-6 col-lg-4" v-for="(item, index) in projects"
        :key = index>
          <div class="projects__card">
            <img :src="item.img.src" :alt="item.img.alt">
            <a href="#">{{ item.link.item }}</a>
            <p>{{ item.description }}</p>
          </div>
        </div>

        <!--        <div class="col-sm-6 col-lg-4">-->
        <!--          <div class="projects__card">-->
        <!--            <img src="/donatelike.png" alt="">-->
        <!--            <a href="#">DonateLike</a>-->
        <!--            <p>Разработали телеграмм-бота для отправки платных реакций</p>-->
        <!--          </div>-->
        <!--        </div>-->
        <!--        -->
        <!--        <div class="col-sm-6 col-lg-4">-->
        <!--          <div class="projects__card">-->
        <!--            <img src="/surgu.png" alt="">-->
        <!--            <a href="#">go.surgu.ru</a>-->
        <!--            <p>Разработали сайт приемной комиссии СурГУ</p>-->
        <!--          </div>-->
        <!--        </div>-->
      </div>
    </div>
  </section>
</template>

<style scoped>

</style>